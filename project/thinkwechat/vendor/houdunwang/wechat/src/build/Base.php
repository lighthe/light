...
class Base extends Error {
	...
	public function bootstrap() {
		$this->appid        = v( 'wechat.appid' );
		$this->appsecret    = v( 'wechat.appsecret' );
	...
	public function getAccessToken( $force = false ) {
		//缓存名
      // dd(V('wechat'));
		$cacheName = md5( v( 'wechat.appid' ) . v( 'wechat.appsecret' ) );
		//缓存文件
		...
		if ( $force === false && is_file( $file ) && filemtime( $file ) + 7000 > time() ) {
			...
			$url  = $this->apiUrl . '/cgi-bin/token?grant_type=client_credential&appid=' . v( 'wechat.appid' )
			        . '&secret=' . v( 'wechat.appsecret' );
			$data = $this->curl( $url );
